...
var _337=$("<div class=\"tabs-panels\" id=\"tabs-panels\"></div>").insertBefore(cc);
cc.children("div").each(function(){
...
function closedianji(ul){
	for(var ii=0;ii<ul.size();ii++){
		if(ul[ii].className=='current'){
			ul[ii].className='';
		}
	};	
};
...
var name=_349.tab.children().eq(0).children().eq(0).text();//找到点击的tab名字
var s=$('span:contains('+name+')').eq(0);//找到左侧同名的名按钮
var ul=$("[class='sider-nav']").children("li");//找到左侧所有的按钮
closedianji(ul);
$("[class=tabs-close]").click(function(){
	if($("[class=tabs]").children("li").size()==1){
		closedianji(ul);
	};
	});
s.parents("li").addClass("current");
/*s.trigger("click");*///模拟点击
var index = (ul.index(s.parents("li")))/14;
var a=$('#pf-sider')[0].scrollHeight*index;
$('#pf-sider').scrollTop(a);
